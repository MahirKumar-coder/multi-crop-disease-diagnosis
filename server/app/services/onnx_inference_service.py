...
class ONNXInferenceService:
    ...
    def _initialize_session(self):
        ...
        # 2. Initialize Calibrated Filter (T=0.85, Threshold=60%)
    ...
    def predict(self, input_tensor: Union[torch.Tensor, np.ndarray]) -> Dict[str, Any]:
        """
        ...
        if isinstance(input_tensor, np.ndarray):
            np_input = input_tensor.astype(np.float32)
        elif hasattr(input_tensor, "cpu"):
            np_input = input_tensor.cpu().numpy().astype(np.float32)
        else:
            np_input = np.array(input_tensor, dtype=np.float32)
        ...
        if self.session is not None:
            ...
            with torch.no_grad():
                if isinstance(input_tensor, torch.Tensor):
                    ...
                    device_tensor = torch.from_numpy(np_input).to(model_service.device)
