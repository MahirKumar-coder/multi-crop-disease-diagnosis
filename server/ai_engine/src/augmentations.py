...
# PyTorch Baseline Transforms (Warmup / Phase 1)
...
def get_base_transforms(img_size: int = 224):
    return transforms.Compose([
        transforms.Resize((img_size, img_size)),
        transforms.ToTensor(),
        transforms.Normalize(mean=IMAGENET_MEAN, std=IMAGENET_STD)
...
# Advanced Albumentations Pipeline (Monday tasks)
def get_albumentations_train_pipeline(img_size: int = 224) -> A.Compose:
    ...
    motion blur, and illumination adjustments.
    """
    return A.Compose([
